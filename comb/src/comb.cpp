...
int main(int argc, char **argv)
{
  ...
  // MPI_THREAD_MULTIPLE für BA
  int required = MPI_THREAD_MULTIPLE; // MPI_THREAD_SINGLE, MPI_THREAD_FUNNELED, MPI_THREAD_SERIALIZED, MPI_THREAD_MULTIPLE
      ...
      if (argv[i][0] == '-')
      {
        ...
        if (strcmp(&argv[i][1], "comm") == 0)
        {
          if (i + 1 < argc && argv[i + 1][0] != '-')
          {
            ...
            else if (strcmp(argv[i], "post_recv") == 0 || strcmp(argv[i], "post_send") == 0 || strcmp(argv[i], "wait_recv") == 0 || strcmp(argv[i], "wait_send") == 0)
            {
...
// IMPORTANT:
#ifdef COMB_ENABLE_MPI
                  comm_avail.mpi_partitioned = enabledisable;
#endif
                ...
                else if (strcmp(argv[i], "mpi_partitioned") == 0)
                {
#ifdef COMB_ENABLE_MPI
                  comm_avail.mpi_partitioned = enabledisable;
#endif
            ...
            else if (strcmp(argv[i], "allow") == 0 || strcmp(argv[i], "disallow") == 0)
            {
    ...
    else if ((ghost_widths[0] < 0 || ghost_widths[1] < 0 || ghost_widths[2] < 0) || (ghost_widths[0] == 0 && ghost_widths[1] == 0 && ghost_widths[2] == 0))
    {
      fgprintf(FileGroup::err_master, "Invalid ghost widths.\n");
      comminfo.abort();
    }
    else if ((divisions[0] != 0 || divisions[1] != 0 || divisions[2] != 0) && (comminfo.size != divisions[0] * divisions[1] * divisions[2]))
    {
...
// IMPORTANT:
#ifdef COMB_ENABLE_MPI
      if (comm_avail.mpi_partitioned)
        COMB::test_cycles_mpi_partitioned(comminfo, info, exec, alloc, num_vars, ncycles, tm, tm_total);
#endif

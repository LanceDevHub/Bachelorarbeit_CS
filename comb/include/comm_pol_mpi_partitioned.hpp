...
#ifndef _COMM_POL_MPI_PARTITIONED_HPP
#define _COMM_POL_MPI_PARTITIONED_HPP
...
// Anzahl der Partitionen angeben
const int partitions = 2;

struct mpi_partitioned_pol
{
    ...
    static const char *get_name() { return "mpi_partitioned"; }
...
template <>
struct CommContext<mpi_partitioned_pol> : MPIContext
{
    using base = MPIContext;

    using pol = mpi_partitioned_pol;
...
namespace detail
{
    ...
    template <>
    struct Message<MessageBase::Kind::send, mpi_partitioned_pol>
        : MessageInterface<MessageBase::Kind::send, mpi_partitioned_pol>
    {
        using base = MessageInterface<MessageBase::Kind::send, mpi_partitioned_pol>;
    ...
    template <>
    struct Message<MessageBase::Kind::recv, mpi_partitioned_pol>
        : MessageInterface<MessageBase::Kind::recv, mpi_partitioned_pol>
    {
        using base = MessageInterface<MessageBase::Kind::recv, mpi_partitioned_pol>;
    ...
    struct MessageGroup<MessageBase::Kind::send, mpi_partitioned_pol, exec_policy>
        : detail::MessageGroupInterface<MessageBase::Kind::send, mpi_partitioned_pol, exec_policy>
    {
        using base = detail::MessageGroupInterface<MessageBase::Kind::send, mpi_partitioned_pol, exec_policy>;
        ...
        void setup(context_type &con, communicator_type &con_comm, message_type **msgs, IdxT len, request_type *requests)
        {
            ...
            LOGPRINTF("%p initialize partitioned communication\n", this);
            for (IdxT i = 0; i < len; ++i)
            {
                ...
                int tag = msg->msg_tag;
                const int num_partitions = partitions;
                IdxT count_per_partition = (msg->nbytes() * this->m_variables.size()) / num_partitions;

                detail::MPI::Psend_init(buf, num_partitions, count_per_partition, MPI_BYTE, partner_rank, tag, con_comm.comm, MPI_INFO_NULL, &requests[i]);
            }
            LOGPRINTF("%p finished initializing partitioned communication\n", this);
        }
        ...
        void pack(context_type &con,
              communicator_type &con_comm,
              message_type **msgs,
              IdxT len,
              detail::Async async,
              request_type *requests)
        {
          ...
          // Parallele Arbeitsgruppe starten
          ...
          for (IdxT i = 0; i < len; ++i)
          {
            ...
            const size_t nmsg = msg->message_items.size(); // 1 oder 2
            const IdxT nvars = static_cast<IdxT>(this->m_variables.size());
    
            const message_item_type *item0 =
                static_cast<const message_item_type *>(msg->message_items[0]);
    
            if (nmsg == 1)
            {
    
              const IdxT nitems = item0->size;
              const IdxT items_per_partition = nitems / partitions;
    
              // Parallel über alle Partitionen
              con.for_all(partitions, [&](IdxT pid)
                          {
    
            // Partition-Bereich bestimmen
            const IdxT start = pid * items_per_partition;
            const IdxT end   = (pid == partitions - 1) ? nitems : (pid + 1) * items_per_partition;
            const IdxT part_len = end - start;
    
    
            if (part_len <= 0)
            {
              detail::MPI::Pready((int)pid, &requests[i]);
              return;
            }
    
            // Buffer-Pointer initialisieren
            char *buf_start = static_cast<char *>(msg->buf)
                            + start * static_cast<IdxT>(sizeof(DataT)) * nvars;
            DataT *buf = reinterpret_cast<DataT *>(buf_start);
    
            // Halo-Indizes holen
            LidxT const *indices = item0->indices + start;
    
            // Über alle Variablen iterieren
            for (IdxT v = 0; v < nvars; ++v)
            {
              DataT const *var = this->m_variables[v];
              // Halo-Zellen in Buffer packen
              for (IdxT k = 0; k < part_len; ++k) *buf++ = var[indices[k]];
            }
    
            // Partition kann senden
            detail::MPI::Pready((int)pid, &requests[i]); });
            }
            else // Edge-Case bei kleiner Domänendekomposition
                //  -> 2 Items statt einem
            {
    
              const message_item_type *item1 =
                  static_cast<const message_item_type *>(msg->message_items[1]);
    
              const IdxT Pitem = partitions / 2;
              const IdxT off1 = item0->nbytes * nvars;
    
              con.for_all(partitions, [&](IdxT pid)
                          {
            const bool second = (pid >= Pitem);
    
            const message_item_type *item = second ? item1 : item0;
            const IdxT item_off = second ? off1 : 0;
    
            const IdxT p_local = second ? (pid - Pitem) : pid;
    
            const IdxT nitems = item->size;
    
            const IdxT start = (p_local * nitems) / Pitem;
            const IdxT end   = ((p_local + 1) * nitems) / Pitem;
            const IdxT part_len = end - start;
            ...
            // Jetzt mit zusätzlichem Offset
            char *buf_start = static_cast<char *>(msg->buf)
                            + item_off
                            + start * static_cast<IdxT>(sizeof(DataT)) * nvars;
            DataT *buf = reinterpret_cast<DataT *>(buf_start);
    
            LidxT const *indices = item->indices + start;
    ...
    template <typename exec_policy>
    struct MessageGroup<MessageBase::Kind::recv, mpi_partitioned_pol, exec_policy>
        : detail::MessageGroupInterface<MessageBase::Kind::recv, mpi_partitioned_pol, exec_policy>
    {
        using base = detail::MessageGroupInterface<MessageBase::Kind::recv, mpi_partitioned_pol, exec_policy>;
        ...
        void setup(context_type &con, communicator_type &con_comm, message_type **msgs, IdxT len, request_type *requests)
        {
            ...
            for (IdxT i = 0; i < len; ++i)
            {
                ...
                int partner_rank = msg->partner_rank;
                int tag = msg->msg_tag;
                // IMPORTANT: analog zur Senderseite
                const int num_partitions = partitions;
                IdxT count_per_partition = (msg->nbytes() * this->m_variables.size()) / num_partitions;
                detail::MPI::Precv_init(buf, num_partitions, count_per_partition, MPI_BYTE, partner_rank, tag, con_comm.comm, MPI_INFO_NULL, &requests[i]);
            }
            LOGPRINTF("%p finished initializing partitioned communication\n", this);
        }
                ...
                detail::Async async)
        {
          ...
          for (IdxT i = 0; i < len; ++i)
          {
            ...
            // Buffer-Pointer initialisieren
            char const *buf_start = static_cast<char const *>(msg->buf)
                                  + start * static_cast<IdxT>(sizeof(DataT)) * nvars;
            DataT const *buf = reinterpret_cast<DataT const *>(buf_start);
            ...
            for (IdxT v = 0; v < nvars; ++v)
            {
              DataT *var = this->m_variables[v];
              // Daten dem Empfangspuffer rauspacken
              for (IdxT k = 0; k < part_len; ++k) var[indices[k]] = *buf++;
            } });
            }
            else // Edges-Case bei 2 items
            {
              const message_item_type *item1 =
                  static_cast<const message_item_type *>(msg->message_items[1]);
    ...
    template <>
    struct MessageGroup<MessageBase::Kind::send, mpi_partitioned_pol, mpi_type_pol>
        : detail::MessageGroupInterface<MessageBase::Kind::send, mpi_partitioned_pol, mpi_type_pol>
    {
        using base = detail::MessageGroupInterface<MessageBase::Kind::send, mpi_partitioned_pol, mpi_type_pol>;
        ...
        // Added request_type *requests as parameter
        void pack(context_type &con, communicator_type &con_comm, message_type **msgs, IdxT len, detail::Async async, request_type *requests)
        {
            if (len <= 0)
                return;
            con.start_group(this->m_groups[len - 1]);
    ...
    template <>
    struct MessageGroup<MessageBase::Kind::recv, mpi_partitioned_pol, mpi_type_pol>
        : detail::MessageGroupInterface<MessageBase::Kind::recv, mpi_partitioned_pol, mpi_type_pol>
    {
        using base = detail::MessageGroupInterface<MessageBase::Kind::recv, mpi_partitioned_pol, mpi_type_pol>;
...
#endif // _COMM_POL_MPI_PARTITIONED_HPP

...
namespace COMB
{
  ...
  {
    return con_mesh_in.available() && con_many_in.available() && con_few_in.available() && aloc_mesh_in.available(AllocatorInfo::UseType::Mesh) && aloc_many_in.available(AllocatorInfo::UseType::Buffer) && aloc_few_in.available(AllocatorInfo::UseType::Buffer) && aloc_many_in.accessible(con_comm_in) && aloc_few_in.accessible(con_comm_in) && aloc_mesh_in.accessible(con_mesh_in.get()) && aloc_mesh_in.accessible(con_many_in.get()) && aloc_many_in.accessible(con_many_in.get()) && aloc_mesh_in.accessible(con_few_in.get()) && aloc_few_in.accessible(con_few_in.get());
  }
                 ...
                 Timer &tm, Timer &tm_total)
  {
    ...
    char test_name[1024] = "";
    snprintf(test_name, 1024, "Comm %s Mesh %s %s Buffers %s %s %s %s",
             pol_comm::get_name(),
...

    {
      ...
      // Wenn mpiP laufen gelassen wird muss "test-comm" auskommentiert werden
      // Sonst werden alle aufrufe doppelt getracked
      tm_total.start(tm_con, "test-comm");

      if (comm_type::partitioned) {
        // tm.start(tm_con, "init-partitioned-comm");
        comm.init_persistent_comm(con_many, con_few);
        // tm.stop(tm_con);
      }
      else if (comm_type::persistent)
      {
      ...
      for (IdxT test_cycle = 0; test_cycle < ntestcycles; ++test_cycle)
      { // test comm
      ...
      if (comm_type::partitioned) {
        // tm.start(tm_con, "cleanup-partitioned-comm");
      ...
      if (comm_type::partitioned)
      {
        tm.start(tm_con, "init-partitioned-commm");
        comm.init_persistent_comm(con_many, con_few);
        tm.stop(tm_con);
      }
      else if (comm_type::persistent)

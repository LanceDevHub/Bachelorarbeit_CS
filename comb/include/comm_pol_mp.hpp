...
namespace detail
{
  ...
  namespace mp
  {
    ...
    struct Request
    {
      ...
      Request()
          : status(0), request(nullptr), context_type(ContextEnum::invalid), context(), completed(true)
      {
      }

      Request(Request const &other)
          : status(other.status), request(other.request), context_type(ContextEnum::invalid), context(), completed(other.completed)
      {

...
namespace detail
{
  ...
  namespace gpump
  {
    ...
    struct Request
    {
      ...
      Request()
          : status(0), g(nullptr), partner_rank(-1), context_type(ContextEnum::invalid), context(), completed(true)
      {
      }

      Request(Request const &other)
          : status(other.status), g(other.g), partner_rank(other.partner_rank), context_type(ContextEnum::invalid), context(), completed(other.completed)
      {
    ...
    struct mempool
    {
      ...
      void remove_allocators(struct gpump *g)
      {
        ...
        if (error)
          throw std::logic_error("can not remove Allocator with used ptr");
        this->g = nullptr;
      }
...
struct CommContext<gpump_pol> : CudaContext
{
  ...
  CommContext(CommContext const &a_, MPI_Comm comm_)
      : base(a_), g(detail::gpump::init(comm_))
  {
  }
  ...
  void wait_request(message_request_type *msg_request)
  {
    ...
    while (1)
    {
      for (message_request_type *other_msg_request : get_message_request_map())
      {
        ...
        if (other_msg_request == msg_request && other_request.completed)
          return;
      }
    }
  }
...
namespace detail
{
      ...
      : detail::MessageGroupInterface<MessageBase::Kind::send, gpump_pol, exec_policy>
  {
    ...
    void pack(context_type &con, communicator_type &con_comm, message_type **msgs, IdxT len, detail::Async async, request_type *requests)
    {
      ...
      else if (false && async == detail::Async::no)
      { // not sure how to know when individual contexts are in different streams
        for (IdxT i = 0; i < len; ++i)
        {
          const message_type *msg = msgs[i];
          char *buf = static_cast<char *>(msg->buf);

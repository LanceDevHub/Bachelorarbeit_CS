...
namespace detail
{
  ...
  namespace MPI
  {
    ...
    // IMPORTANT: Partitioned spezifische Wrapper
    // count_per_part: Elemente pro Partition
    inline void Psend_init(const void *buf, int partitions, MPI_Count count_per_part, MPI_Datatype mpi_type, int dest, int tag, MPI_Comm comm, MPI_Info info, MPI_Request *request)
    {
      int ret = MPI_Psend_init(buf, partitions, count_per_part, mpi_type, dest, tag, comm, info, request);
      assert(ret == MPI_SUCCESS);
    }

    inline void Precv_init(void *buf, int partitions, MPI_Count count_per_part, MPI_Datatype mpi_type,
                           int src, int tag, MPI_Comm comm, MPI_Info info, MPI_Request *request)
    {
      int ret = MPI_Precv_init(buf, partitions, count_per_part, mpi_type, src, tag, comm, info, request);
      assert(ret == MPI_SUCCESS);
    }

    inline void Pready(int partition, MPI_Request *request)
    {
      int ret = MPI_Pready(partition, *request);
      assert(ret == MPI_SUCCESS);
    }

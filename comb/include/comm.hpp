...
#endif
        ,
        size(0), divisions{0, 0, 0}, periodic{0, 0, 0}
  {
  }
...
#endif
        ,
        size(other.size), divisions{other.divisions[0], other.divisions[1], other.divisions[2]}, periodic{other.periodic[0], other.periodic[1], other.periodic[2]}
  {
  }
    ...
    for (IdxT dim = 0; dim < 3; ++dim)
    {
      ...
      if (periodic[dim])
      {
        ...
        if (input_coords[dim] < 0)
          input_coords[dim] += divisions[dim];
      }
...
struct CommInfo
{
  ...
  static const char *method_str(method m)
  {
    ...
    case method::testany:
      str = "test_any";
      break;
    case method::waitsome:
      str = "wait_some";
      break;
    case method::testsome:
      str = "test_some";
      break;
    case method::waitall:
      str = "wait_all";
  ...
  CommInfo()
      : rank(-1), size(0), cart(), cutoff(200), post_send_method(method::waitall), post_recv_method(method::waitall), wait_send_method(method::waitall), wait_recv_method(method::waitall)
  {
#ifdef COMB_ENABLE_MPI
...
struct Comm
{
  ...
  // IMPORANT: Added partitioned flag
  static const bool partitioned = policy_comm::partitioned;
  ...
  struct send_message_vars_s
  {
    send_message_vars_s(COMB::Allocator &many_aloc_, COMB::Allocator &few_aloc_)
        : message_group_many(many_aloc_), message_group_few(few_aloc_)
    {
    }
  ...
  Comm(CommContext<policy_comm> &con_comm_, CommInfo &comminfo_,
      ...
      : mesh_aloc(mesh_aloc_), many_aloc(many_aloc_), few_aloc(few_aloc_), comminfo(comminfo_), con_comm(con_comm_), post_recv_method(comminfo_.post_recv_method), post_send_method(comminfo_.post_send_method), wait_recv_method(comminfo_.wait_recv_method), wait_send_method(comminfo_.wait_send_method), m_sends(many_aloc_, few_aloc_), m_recvs(many_aloc_, few_aloc_)
  {
  ...
  void postSend(ExecContext<policy_many> &con_many, ExecContext<policy_few> &con_few)
  {
    ...
    case CommInfo::method::waitany:
    {
      ...
      for (IdxT i_many = 0; i_many < num_many; i_many++)
      {
        ...
        // IMPORTANT: Reihenfolge von pack, Isend muss an partitioned angepasst werden
        if (partitioned)
        {
          m_sends.message_group_many.Isend(con_many, con_comm, &messages_many[i_many], 1, detail::Async::no, &requests_many[i_many]);
          // IMPORTANT: Added requests to pack method
          m_sends.message_group_many.pack(con_many, con_comm, &messages_many[i_many], 1, detail::Async::no, requests_many);
      ...
      for (IdxT i_few = 0; i_few < num_few; i_few++)
      {
        ...
        if (partitioned)
        {
          ...
          // IMPORTANT: Added requests to pack method
          m_sends.message_group_few.pack(con_few, con_comm, &messages_few[i_few], 1, detail::Async::no, requests_few);
    ...
    case CommInfo::method::testany:
    {
      ...
      while (post_many_send < num_many || post_few_send < num_few)
      {
        ...
        {
          // IMPORTANT: Added requests to pack method
          m_sends.message_group_many.pack(con_many, con_comm, &messages_many[pack_many_send], 1, detail::Async::yes, requests_many);
          ++pack_many_send;
        }
        else if (pack_few_send < num_few)
        {
          // IMPORTANT: Added requests to pack method
          m_sends.message_group_few.pack(con_few, con_comm, &messages_few[pack_few_send], 1, detail::Async::yes, requests_few);
          ++pack_few_send;
        }
    ...
    case CommInfo::method::waitsome:
    {
      ...
      if (partitioned)
      {
        ...
        // IMPORTANT: Added requests to pack
        m_sends.message_group_many.pack(con_many, con_comm, &messages_many[0], num_many, detail::Async::no, requests_many);
      ...
      if (partitioned)
      {
        ...
        // IMPORTANT: Added requests to pack method
        m_sends.message_group_few.pack(con_few, con_comm, &messages_few[0], num_few, detail::Async::no, requests_few);
    ...
    case CommInfo::method::testsome:
    {
      ...
      while (post_many_send < num_many || post_few_send < num_few)
      {
        ...
        {
          // IMPORTANT: Added requests to pack method
          m_sends.message_group_many.pack(con_many, con_comm, &messages_many[pack_many_send], num_many - pack_many_send, detail::Async::yes, requests_many);
          pack_many_send = num_many;
        }
        else if (pack_few_send < num_few)
        {
          // IMPORTANT: Added requests to pack method
          m_sends.message_group_few.pack(con_few, con_comm, &messages_few[pack_few_send], num_few - pack_few_send, detail::Async::yes, requests_few);

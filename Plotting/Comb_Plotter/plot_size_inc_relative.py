...
plt.plot(
    sizes_arr, speedup_pers,
    marker="s", markersize=4, linewidth=2,
    color="tab:orange", label="Persistent vs. Non-Blocking",
)
plt.plot(
    sizes_arr, speedup_part,
    marker="^", markersize=4, linewidth=2,
    color="tab:green", label="Partitioned vs. Non-Blocking",
)

plt.xlabel("Größe einer Halo-Fläche [KiB]")
plt.ylabel("Speedup gegenüber MPI Non-Blocking [%]")
# plt.title("Size Increase Scaling: Speedup (prozentuale Ersparnis)")

plt.grid(True, which="both", linestyle="--", alpha=0.5)
plt.legend()
plt.tight_layout()

plt.savefig("size_scaling_speedup_bench_comm.png", dpi=200)
# plt.show()
